            ...
            for (int k3 = 0;k3<(int)midPoint_pair.size();++k3)
            {
                ...
                if(distance >= distance_max)//|| slope <= slope_min)
                {
                    if(slope <= 0.26)
                    {
                        ...
                        g_x_buff = mid_point.x;    //更新缓存帧
                        g_y_buff = mid_point.y;    //更新缓存帧
                ...
                switch (Recoginition_FLAG)
                {
                    case 0:
                    {
                        ...
                        if(SuccessSend == true)
                        {
                            ...
                            {
                                int buff_reduce_value_x = abs(src_img.cols/2-g_x_buff);
                                int buff_reduce_value_y = abs(src_img.rows/2-g_y_buff);
                                int send_buff_x;
                                int send_buff_y;
                                if(g_x_buff < src_img.cols/2)
                                {
                                    send_buff_x = g_x_buff + buff_reduce_value_x/(bufftime-SendBuf_COUNT);
                                }
                                else
                                {
                                    send_buff_x = g_x_buff - buff_reduce_value_x/(bufftime-SendBuf_COUNT);
                                }
                                if(g_y_buff < src_img.rows/2)
                                {
                                    send_buff_y = g_y_buff + buff_reduce_value_y/(bufftime-SendBuf_COUNT);
                                }
                                else
                                {
                                    send_buff_y = g_y_buff - buff_reduce_value_y/(bufftime-SendBuf_COUNT);
                                }
                                Port.RMSerialWrite(send_buff_x,send_buff_y,0);
                                SendBuf_COUNT += 1;
                            ...
                            else
                            {
                                ...
                                switch(leftorright)
                                {
                                    ...
                                    case 2:
                                    {
                                        if(none_count < 200)
                                        {
                                            Port.RMSerialWrite(X_Widht,Y_height,2);
                                    ...
                                    case 3:
                                    {
                                        if(none_count < 200)
                                        {
                                            Port.RMSerialWrite(X_Widht,Y_height,3);
